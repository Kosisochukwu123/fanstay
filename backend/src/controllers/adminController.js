...
exports.getPendingHostApplications = asyncHandler(async (req, res) => {
  const users = await User.find({ hostApprovalStatus: 'pending' }).select('-password');
...
exports.getAllProperties = asyncHandler(async (req, res) => {
  ...
  const [properties, total] = await Promise.all([
    Property.find(query).populate('host', 'name email').sort('-createdAt').skip(skip).limit(Number(limit)),
    Property.countDocuments(query),
  ]);

  res.status(200).json({ success: true, count: properties.length, total, page: Number(page), pages: Math.ceil(total / Number(limit)), properties });
});
...
exports.updateGiftCardProvider = asyncHandler(async (req, res) => {
  const provider = await GiftCardProvider.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
