...
exports.createCryptoCharge = asyncHandler(async (req, res) => {
  ...
  if (booking.paymentMethod !== 'crypto') {
    return res.status(400).json({ success: false, message: 'Booking is not set up for crypto payment' });
  }
...
exports.coinbaseWebhook = asyncHandler(async (req, res) => {
  ...
  if (eventType === 'charge:confirmed') {
    ...
    const confirmedEvent = timeline.find((t) => t.status === 'COMPLETED' || t.status === 'CONFIRMED');
    payment.transactionHash = confirmedEvent?.payment?.transaction_id || '';
    await payment.save();

    const booking = await Booking.findById(payment.booking).populate('property').populate('guest');
...
exports.submitGiftCard = asyncHandler(async (req, res) => {
  ...
  if (booking.paymentMethod !== 'gift_card') {
    return res.status(400).json({ success: false, message: 'Booking is not set up for gift card payment' });
  }

  // Step 3: Validate provider is supported/active
  const providerDoc = await GiftCardProvider.findOne({ name: provider, isActive: true });
  if (!providerDoc) {
    return res.status(400).json({ success: false, message: 'Gift card provider not supported' });
  }
...
exports.getPendingGiftCards = asyncHandler(async (req, res) => {
  const payments = await Payment.find({ paymentProvider: 'gift_card', status: 'awaiting_admin_review' })
    .select('+giftCardCode')
    .populate('booking')
...
exports.getPaymentByBooking = asyncHandler(async (req, res) => {
  const payment = await Payment.findOne({ booking: req.params.bookingId }).sort('-createdAt');
